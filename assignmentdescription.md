...
structured program and implement the program in the Swift programming language. The ﬁnal
product shall be thoroughly tested and ready to be submitted to the App Store.
42889 41889
Important Note: Use SwiftUI only when implementing the assignment. Storyboard is not allowed.
...
iOS UI programming
Eﬀective use of platform APIs
Program testing
Functionality Speciﬁcation
The game is called BubblePop (you can call your app a diﬀerent name if you wish to). In this
...
points. Bubbles come in ﬁve colours: red, pink, green, blue and black. Each colour corresponds
to a speciﬁc number of points and has a speciﬁc probability of appearance (see table 1). All
bubbles appear on the screen brieﬂy (see core functionality 9). A player needs to pop as many
...
board is displayed after a game run is ﬁnished. Figure 1 provides a rough mock up of what the
...
for each functionality item speciﬁed below.
Table 1. Speciﬁcations for coloured bubbles
...
with some parts oﬀ the screen once it has fully appeared.
...
the bubbles after the ﬁrst one will get 1.5 times their original game points. For example, if
...
10. When the game stops, the player’s score shall be saved in a persistent ﬁle (or database)
...
Extended Functionalities (please do not start on these until ﬁnishing CFs)
1. In addition to core function 7 and 9, displayed bubbles shall move and go oﬀ the screen if
...
2. Bubbles shrinking, growing, or ﬂying away when removed or tapped
...
PROGRAM/TEST HINT
Make sure your program works under diﬀerent iOS devices/simulators (e.g. iPhone4s, iPad
etc.) with diﬀerent screen sizes and orientations.
...
2 marks for correct display on diﬀerent screen sizes and orientations.
Code Style: 3 marks
...
Data modeling: Do the data structures reﬂect the problem domain?
...
Please note: Regardless of how many times you have submitted your project, if your ﬁnal
...
adversely aﬀected their performance.
Bug reports
It is quite possible that errors or ambiguities may be found in the speciﬁcation. If so, updates
...
2. It must be reported on the Canvas discussion board to be accepted.
3. It must be a genuine bug. By genuine I mean it requires me to amend the material.
4. If a number of students post a report on the same bug, the ﬁrst who posted will receive
the mark.
Return of Assessed Project
It is expected that marks will be made available one week after the ﬁnal due date via
...
understanding of the subject material covered. It's not about being able to ﬁnd solutions
on the Internet.
